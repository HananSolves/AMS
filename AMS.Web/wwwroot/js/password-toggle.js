// Reusable password toggle for login/register pages
function togglePassword(inputId, pathId) {
    // If called without params (legacy), default to common ids used by Login view
    if (!inputId) inputId = 'password';
    if (!pathId) pathId = 'eye-path';

    const passwordInput = document.getElementById(inputId);
    const eyePath = document.getElementById(pathId);
    if (!passwordInput || !eyePath) return;
...
// Expose globally (for older browsers that may not support modules)
